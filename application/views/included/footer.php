...
<div class="modal fade" id="crimeReport" tabindex="-1" role="dialog" aria-labelledby="ModalLabel"
 aria-hidden="true" style="position: absolute; " >
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="overflow: auto;height: 90vh;">
      ...
      <div class="modal-body">
          <div class="row">
              <div class="col-sm-6">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" placeholder="<?php echo $this->lang->line('Your Name');?>" name="contact_name_crime" id="crime_name">
            ...
            <div class="row">
            <div class="col-sm-6">
              <label for="recipient-name" class="col-form-label"><?php echo $this->lang->line('Accuser Name')?></label>
              <input type="text" class="form-control" placeholder="የተጠርጣሪው ስም" name="accuser_name" id="accuser_name">
              <div class="accuser_name" style="color:red"></div>
          </div>
            <div class="col-sm-6">
              <label for="recipient-name" class="col-form-label"><?php echo $this->lang->line('Crime Type')?></label>
         ...
         <?php } ?>
            ...
            <div class="row">
            <div class="col-sm-6">
              <label for="recipient-name" class="col-form-label"><?php echo $this->lang->line('Region');?>
            ...
            <div class="col-md-6">
              <label for="recipient-name" class="col-form-label"><?php echo $this->lang->line('Location');?>  <i style="color:red" class="icon icon_question_alt2"></i></label>
              <input type="text" class="form-control" placeholder="e.g city, subcity" name="subcity" id="subcity">
              <div class="subcity" style="color:red"></div>
          </div>
          <div class="col-md-3">
            <div class="form-group">
              <label for="recipient-name" class="col-form-label" style="margin-top: 25%; margin-left: 4%;" > <?php echo $this->lang->line('Code')?> <span class="icon_refresh" aria-hidden="true" onclick="reloadPin()"></span>
              </label>
          ...
          <div class="col-md-9">
            <div class="form-group">
              <label></label>
              <input name="code" type="text" class="form-control" placeholder="ኮዱን እዚህ ጋር ያስገቡ" id="inputedCode" onkeyup="pincode()">
          ...
          <div class="form-group">
            <label for="message-text" class="col-form-label"><?php echo $this->lang->line('Detail Report');?></label>
            <textarea class="form-control" id="crime_message" rows="5" placeholder="ጥቆማ ...." name="report_crime"></textarea>
            <div class="crime_message" style="color:red"></div>
          </div>
          <div class="form-group" style="padding-left:200px">
            <label for="message-text" class="col-form-label">
              <img data-rotate="90" id="user_pro" src="<?php echo base_url('assets/default_logo.png');?>" style="min-width: auto; min-height: auto; max-width: 90%; max-height: 60px; margin-left: 5%;">
            </label>
            <div class="image-upload" >
            <label for="add_user_photo"  style="cursor: pointer;">
            <sapn class ="icon-upload " style="border-radius:10px;border:1px solid;"> <?php echo $this->lang->line('Accuser Photo')?>&nbsp; </span></label>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-border" onclick="crimeReport()"><?php echo $this->lang->line('Report')?> <span class="click_report"></span></button>
      </div>
    </form>
    </div>
  </div>
...
<div class="modal fade" id="commentForm" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalLabel">አስተያየት መሰብሰቢያ ቅጽ</h5>
      ...
      <div class="modal-body">
          <div class="form-group">
            ...
            <input type="email" class="form-control" placeholder="<?php echo $this->lang->line('Your Email')?>" name="comment_email" id="message_email">
            <div class="response_email" style="color:red"></div>
          </div>
          <div class="form-group">
            <label for="message-text" class="col-form-label"><?php echo $this->lang->line('Message')?></label>
            <textarea class="form-control" id="message_text" rows="5" placeholder="<?php echo $this->lang->line('Message')?>" name="comment_message"></textarea>
            <div class="response_message" style="color:red"></div>

          </div>
      </div>
      <div class="modal-footer">
     <button type="submit" class="btn btn-border" onclick="saveComment()"><?php echo $this->lang->line('Send Message')?><span class="checkAsFinishClicked"></span></button>
      </div>
    </form>
    </div>
  </div>
...
<script type="text/javascript">
  function language(){
    ...
    window.location.replace("<?php echo base_url('top_most_wanted');?>");
...
function filterByCrimeType(by_value,type){
  ...
  loadFugitive(1,by_value,type);
  topMostWanted(1,by_value,type);
}
}
function filterDataByName(){
  ...
  if(name){
    $('#filter_by_crime').prop('selectedIndex', 0);
    $('#filter_by_year').prop('selectedIndex', 0);
    loadFugitive(1,name,'name');
    topMostWanted(1,name,'name');
  }
  else{
...
<script type="text/javascript">
  function  aggrement(which) {
      ...
      SwalWithBootstrapButtons.fire({
        title: '<strong> <i class="icon icon_error-oct_alt"></i>  <?php echo $this->lang->line('Warning');?> </strong>',
        showClass: {
        ...
        html: '<p style="text-align:left;"><br> <small> <?php echo $this->lang->line('You are entering a EFDRE geovermental website');?> </small> <br> <small><?php echo $this->lang->line('I understand that entering repetative information will result in being not to be considered');?> </small><small><?php echo $this->lang->line('understand that providing false information could subject in legal consequences according to EFDRE')?> </small></p>',
        showCancelButton: true,
        confirmButtonText: '<?php echo $this->lang->line('Agree');?>',
        cancelButtonText: '<?php echo $this->lang->line('Disagree');?>',
          ...
          return fetch(``)
            .then(response => {
              if (response.ok) {
                ...
                swal("Cancelled", $('.close_report').click());
...
<script>
$(function(){
loadFugitive(1,'all','all');
topMostWanted(1,'all','all');
})
...
result+='<a title="Read More" href="<?php echo base_url('frontController/get_single_view?id=')?>'+data[take].crdoc_id+'">';
result+='<div class="col-xs-12 col-sm-6 col-md-3 mb-30">';
   result+='<div class="box-hover-effect effect2 bg-lightest-gray wow fadeInUp maxwidth400 p-10" data-wow-duration="1.5s" data-wow-offset="10" style="box-shadow: 0 5px 35px -16px #888888; height: 340px;">';
    ...
    result+='<h6 align="center" style="color: #fff;"><b> <?php echo $this->lang->line('Reward');?> </b></h6>';
  result+='</div>';
}
      ...
      result+='<div class="overlay black">';
        result+='<div class="display-table">';
          result+='<div class="display-table-cell">';
            ...
            result+='<tbody style="text-align: justify;">';
            result+='<tr>';
            result+='<td>';
              result+='&nbsp; <?php echo $this->lang->line('Name');?>:&nbsp; ';
              result+='</td>';
              result+='<td> &nbsp;';
              result+='</td>';
              result+='<td>';
              result+=(data[take].criminal_name).toUpperCase();
              result+='</td>';
              result+='</tr>';
              result+='<tr>';
              result+='<td>';
              result+='&nbsp; <?php echo $this->lang->line('Crime');?>:&nbsp; ';
              result+='</td>';
              result+='<td>';
              result+='</td>';
              result+='<td>';
              result+=data[take].criminal_type;
              result+='</td>';
              result+='</tr>';
              result+='<tr>';
              result+='<td>';
              result+='&nbsp; <?php echo $this->lang->line('Age');?>:&nbsp; ';
              result+='</td>';
              result+='<td>';
              result+='</td>';
              result+='<td>';
              result+=data[take].criminal_birthdate;
              result+='</td>';
              result+='</tr>';
              result+='<tr>';
              result+='<td>';
              result+='&nbsp; <?php echo $this->lang->line('Height');?>:&nbsp; ';
              result+='</td>';
              result+='<td>';
              result+='</td>';
              result+='<td>';
              result+=data[take].criminal_height;
              result+='</td>';
              result+='</tr>';
              result+='</tbody>';
              result+='</table>';
    ...
    result+='</div>';
  result+='</div>';
  result+='<h5 align="center" style="margin-top:-11%;"> '+(data[take].criminal_name).toUpperCase()+'</h5>';
result+='</div>';
result+='</a>';
}
